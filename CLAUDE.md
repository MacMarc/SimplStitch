...
- [x] Import/Export (Phase 7)
  ...
  - **Bugfix (GitHub Issue #16, nach Live-Test):** Der eigentliche Grund für "zu wenige Stiche/nur 1 Farbe trotz mehrerer Formen im Export" war kein Fehler in `FileExportService`s Kombinationslogik selbst (die iteriert schon immer korrekt über alle sichtbaren Objekte), sondern dass frisch gezeichnete `DesignObject`s seit jeher **kein** `StitchSettings` bekamen — `CanvasStore.makeShapeObject`/`makePathObject` liessen `stitchSettings` auf `nil`, und `FileExportService.stitchableObjects(from:)` filtert genau danach (`$0.stitchSettings != nil`). Ohne manuellen Besuch im Objekt-Inspektor pro Form blieb eine Form dauerhaft unstickbar und fiel beim Export lautlos weg. Fix: `CanvasStore` vergibt jetzt beim Erzeugen sofort sinnvolle Default-Sticheinstellungen — Rechteck/Kreis/Stern `StitchSettings()`-Defaults (Tatami), Freihand-Pfad `.straight` (Laufstich, da typischerweise eine offene Linie). Text bekommt bewusst weiterhin keine Default-Einstellungen (Text-zu-Stich-Konvertierung ist nicht implementiert, siehe Phase 5d). Zusätzlich: `FileExportService.combinedStitchesAndThreads` bricht nicht mehr den gesamten Export ab, wenn die Stichgenerierung für ein einzelnes Objekt fehlschlägt — dieses Objekt wird jetzt übersprungen statt den kompletten Export scheitern zu lassen. Neuer Regressionstest `FileExportServiceTests.realBridgeExportsShapesDrawnViaCanvasStoreWithoutManualInspectorVisit` zeichnet zwei Formen exakt wie über die UI (ohne Inspector-Zutun) und verifiziert gegen die echte Bridge, dass beide Farben im Export ankommen. Die im selben Issue aufgeworfene Design-Frage (Formen gruppieren, ist ein Layer pro Form sinnvoll?) ist davon unabhängig und bleibt offen
- [x] UI (Toolbar + Menü) (Phase 8, in Unteraufgaben wie Phase 5)
  ...
  - [x] 8b Menüleiste
    - `CanvasStore.deleteObject(_:)`/`deleteSelectedObject()` neu — bislang gab es überhaupt keine Möglichkeit, ein Objekt vom Canvas zu entfernen (Phase 5 hatte Erzeugen/Verschieben/Skalieren/Z-Order, aber kein Löschen). Bereinigt Selektion/Text-Bearbeitungszustand/aktiven Transform-Drag, falls sie sich auf das gelöschte Objekt bezogen; gesperrte Objekte lassen sich bewusst löschen (Sperre verhindert nur Verschieben/Skalieren/Drehen, wie beim Z-Order-Umsortieren in 5e)
    - `FocusedValues+Canvas.swift`: `.commands` wird auf Scene-Ebene deklariert (`SimplStitchApp`), hat also keinen direkten Zugriff auf den `canvasStore`, der pro Fenster in `ContentView` lebt — der von Apple vorgesehene Weg dafür ist `@FocusedValue`/`.focusedSceneValue`. Vier Werte: `canvasStore`, `isExportDialogPresented`/`isLayersPanelPresented` (Bindings für die bestehenden Sheet-/Inspector-Toggles), `zoomToFitAction` (ein Closure, das `CanvasView` bereitstellt, da nur diese die aktuelle `GeometryReader`-Viewportgrösse kennt — `CanvasStore.zoomToFit(viewportSize:)` selbst nimmt keine Geometrie entgegen, die man ohne View-Kontext ermitteln könnte)
    - `SimplStitchCommands.swift`: `DocumentGroup` liefert Ablage (Neu/Öffnen/Zuletzt benutzt/Schliessen/Sichern/Duplizieren/…) und die Grundgerüste für Bearbeiten (Rückgängig/Wiederholen/Ausschneiden/Kopieren/Einfügen/Alles auswählen) sowie Fenster/Hilfe automatisch mit. Ergänzt: "Exportieren…" (⌘E, nach der Sichern-Gruppe im Ablage-Menü), "Objekt löschen" (Delete-Taste, Bearbeiten-Menü), eigenes `CommandMenu` "Werkzeug" (alle 6 `CanvasTool`-Fälle), eigenes `CommandMenu` "Objekt" (Z-Order ⌘⇧]/⌘]/⌘[/⌘⇧[, Ein-/Ausblenden und Sperren/Entsperren mit dynamischem Titel je nach aktuellem Objektzustand, wiederverwendet dieselben `layers.*`-Lokalisierungsschlüssel wie das Ebenen-Panel), Ergänzungen im Darstellung-Menü (Vergrössern/Verkleinern/Originalgrösse/Ganze-Seite-einpassen, Ebenen-Panel-Toggle). Alle neuen Menüpunkte deaktivieren sich korrekt, wenn kein Dokumentfenster im Fokus ist bzw. keine Aktion sinnvoll anwendbar ist (z.B. Z-Order ohne Selektion)
    - **Namenskollision entdeckt und behoben:** `CommandGroup(after: .pasteboard)` fügt einen eigenen "Löschen"-Eintrag nach der von SwiftUI automatisch bereitgestellten Standard-Textbearbeitungsgruppe ein, die selbst BEREITS einen generischen "Löschen"-Eintrag enthält (für Textfeld-Editing) — zwei identisch benannte Menüpunkte direkt untereinander. Behoben durch Umbenennen auf "Objekt löschen" (EN: "Delete Object")
    - **Beobachtung (kein Bug):** Die Z-Order-Tastenkürzel (⌘⇧]/⌘]/⌘[/⌘⇧[) werden in der Menüleiste auf einer deutschen Tastatur als "⇧⌘Ä"/"⌘Ä"/"⌘Ö"/"⇧⌘Ö" dargestellt — macOS zeigt `NSMenuItem`-Tastenkürzel-Symbole immer relativ zum aktuell aktiven Tastatur-Layout an, nicht das literale Zeichen aus dem Code. Normales, von vielen internationalisierten Mac-Apps geteiltes Verhalten, kein Fehler
    - Scope-Hinweis: kein echtes Undo/Redo für Canvas-Mutationen (Verschieben, Skalieren, Farbe ändern, Löschen, …) — "Rückgängig/Wiederholen" im Bearbeiten-Menü bleibt das von `DocumentGroup`/AppKit automatisch bereitgestellte, aber inhaltsleere Grundgerüst. Jede `DesignObject`-Mutation müsste eine `UndoManager`-Registrierung bekommen — ein eigenständiges, nicht-triviales Feature, bewusst nicht Teil von 8b
    - Tests: 5 neue `CanvasStoreTests` für `deleteObject`/`deleteSelectedObject` (entfernt Objekt + bereinigt Selektion, lässt andere Selektionen unangetastet, kein Objekt selektiert → no-op, funktioniert auch bei gesperrten Objekten) — grün (92 Tests gesamt). Keine dedizierten Tests für `SimplStitchCommands`/FocusedValues selbst (SwiftUI-Menü-/Fokus-Lebenszyklus lässt sich nicht sinnvoll headless testen)
    - Visuell verifiziert (`run-and-screenshot`-Skill): alle sechs Menüs per AppleScript-Klick geöffnet und geprüft — Werkzeug-Menü zeigt alle 6 Werkzeuge (deaktiviert ohne offenes Dokument, aktiv sobald eins offen ist), Objekt-Menü komplett deaktiviert ohne Selektion, Darstellung-Menü mit allen vier Zoom-Aktionen + Ebenen-Panel-Toggle, Ablage-Menü mit "Exportieren…" nach der Sichern-Gruppe, Bearbeiten-Menü mit sauber getrenntem "Löschen"/"Objekt löschen"
  - [x] 8c Toolbar
    - `CanvasTool.systemImageName` neu (SF Symbols: `cursorarrow`/`rectangle`/`circle`/`star`/`scribble`/`character.cursor.ibeam`) — zentral im Modell statt in der View verstreut, analog zu `displayName`
    - Der provisorische Segmented-Picker in `ContentView` (5b) ist einer echten `ToolbarItemGroup(placement: .principal)` mit sechs eigenen Buttons gewichen — dieselben `CanvasTool`-Fälle wie das Werkzeug-Menü (8b). `Label(...).labelStyle(.titleAndIcon)` erzwingt Icon+Text auch in der Toolbar, wo SwiftUI/macOS eine `Label` sonst standardmässig aufs blosse Icon reduziert (mit Tooltip als Fallback) — CLAUDE.md verlangt hier explizit "Icon + Text-Label (kein Icon-Raten)", das überschreibt bewusst das sonst übliche macOS-Standardverhalten für Toolbar-Labels
    - Aktives Werkzeug bekommt `.buttonStyle(.borderedProminent)`, alle anderen `.bordered` — als `if`/`else` auf Button-Ebene, nicht als Ternary zwischen den beiden Styles (unterschiedliche konkrete `ButtonStyle`-Typen lassen sich nicht direkt in einem Ausdruck vereinen; ein erster Versuch mit `.buttonStyle(...)` + `.tint(...)` kompilierte zwar, brachte aber keine klar sichtbare Hervorhebung — visuell verifiziert und durch die if/else-Variante ersetzt)
  ...
  - [x] 8d Seitenleiste: Objekt-Eigenschaften, Sticheinstellungen, Farbe
    - `StitchDevPanelView` (Phase 6e, seit jeher als "provisorisch bis Phase 8 einen echten Settings-Inspector bringt" dokumentiert) vollständig entfernt und durch `ObjectInspectorView.swift` ersetzt — inkl. der dort bewusst ausgelassenen Unterlage-Auswahl (`UnderlayType`-Picker), die jetzt nachgeholt ist
    - `ObjectInspectorView`: drei `Form`-Sektionen für das selektierte Objekt — "Objekt" (Name, Position X/Y, Grösse W/H, Rotation, Eckenradius nur bei Rechtecken), "Farbe" (`ColorPicker` auf `fillColorHex`), "Sticheinstellungen" (Stichtyp/Dichte/Winkel wie im alten Dev-Panel, plus neu Unterlage). Transform-Änderungen lösen wie Drag-Handles `store.refreshStitchPreview()` aus, da sich die Geometrie ändert
    - `Color+Hex.swift`: `Color(hex:)`/`Color.hexString` — nutzt `CGColor.fromHex` (PreviewImageRenderer.swift) für die Parsing-Richtung statt sie zu duplizieren, `hexString` liest direkt `cgColor.components` (kein `Color.resolve(in:)`, das bräuchte ein `EnvironmentValues`, das ausserhalb einer View nicht sauber verfügbar ist)
    - **Vereinfachung:** kein Eingabefeld für Verzerren (Skew) trotz vorhandener Modellfelder — es gibt weiterhin keinen interaktiven Griff dafür und auch das Rendering wendet es nicht an (unveränderter Scope-Hinweis seit 5c); ein Zahlenfeld ohne sichtbare Wirkung wäre irreführend
    - `CanvasInspectorView.swift` (neu): kombiniert Ebenen-Panel (5e) und Objekt-Inspektor in einem einzigen `.inspector`-Bereich über einen Segmented-Picker ("Ebenen"/"Eigenschaften") — SwiftUI erlaubt nur einen Inspector pro Fenster, ähnlich den Format/Arrange-Tabs in Pages/Keynote. Wechselt automatisch zu "Eigenschaften", sobald ein Objekt selektiert wird (`.onChange(of: store.selectedObjectID)`), Nutzer können jederzeit manuell zurück zu "Ebenen"
    - Der bisherige "Ebenen-Panel"-Toggle (Toolbar-Button, Darstellung-Menü, `@State`/`FocusedValue`) ist konsequent auf "Inspector" umbenannt (`isLayersPanelPresented` → `isInspectorPresented`, `layers.panel.toggle` → `inspector.toggle`, Icon `square.3.layers.3d` → `sidebar.trailing`), da der Bereich jetzt mehr als nur Ebenen zeigt — betrifft `ContentView.swift`, `FocusedValues+Canvas.swift`, `SimplStitchCommands.swift`
    - Tests: neue `ColorHexTests` (Roundtrip, Gross-/Kleinschreibung normalisiert, ungültiger Hex-String → nil, Fallback auf Schwarz für nicht auflösbare Farben) — grün (96 Tests gesamt). Keine UI-Tests für `ObjectInspectorView` selbst (reine Bindings auf bereits getestete Store-/Model-Properties, kein neues Store-Verhalten ausser den bereits vorhandenen `refreshStitchPreview`/Stitch-Settings-Bindings aus dem alten Dev-Panel)
    - **Bekannte Einschränkung bei der visuellen Verifikation:** ohne Drag-Event-Tool (siehe Scope-Hinweis seit Phase 6) lässt sich keine Form zeichnen, daher wurde nur der leere Zustand ("Kein Objekt ausgewählt") visuell bestätigt, nicht der befüllte Inspektor mit echten Objektdaten — dessen Bindings sind aber dieselben, bereits in Phase 6e/6f real gegen die Bridge getesteten Muster (Stichtyp/Dichte/Winkel), nur um Transform- und Farb-Bindings ergänzt
    - Visuell verifiziert (`run-and-screenshot`-Skill): Inspector öffnet mit Segmented-Picker "Ebenen"/"Eigenschaften", "Ebenen" zeigt weiterhin korrekt den leeren Zustand, "Eigenschaften" zeigt den neuen leeren Zustand "Kein Objekt ausgewählt" mit Beschreibungstext
  - [x] 8e Garnlisten-Panel (Paletten-Import + Farbzuweisung per Drag)
    - Dritter Inspector-Tab "Garnlisten" neben Ebenen/Eigenschaften (`CanvasInspectorView` erweitert) — `ThreadPalettesPanelView.swift` mit `@Query(sort: \ThreadPalette.name)`: anders als `StitchProject`/`DesignObject` (Phase 8a, in-memory pro Dokument) sind `ThreadPalette`/`ThreadColor` Teil des echten, projektübergreifenden `sharedModelContainer` — `@Query` liest hier also aus der tatsächlich auf Disk persistierten SwiftData-Datenbank
    - Import über `.fileImporter` (native SwiftUI-Dateiauswahl, gefiltert auf `.gpl`) → `GPLPaletteImporter.importPalette(at:)` (Phase 7) → `modelContext.insert(palette)`. Security-Scoped-Resource-Zugriff (`startAccessingSecurityScopedResource`/`stopAccessingSecurityScopedResource`) beachtet, da App-Sandbox aktiv ist. Fehlerhafte Dateien zeigen einen Alert statt stillschweigend zu scheitern
    - Farbzuweisung per Drag: `DraggedThreadColor` (Codable-`Transferable`, `.json`-Content-Type — kein eigener UTType nötig, da rein appintern) transportiert nur Werte, keine SwiftData-Objektreferenz über den Drag-Vorgang hinweg (bräuchte einen ModelContext am Drop-Ort). `CanvasView` hat einen `.dropDestination(for: DraggedThreadColor.self)`, der die Drop-Position in Design-Koordinaten umrechnet und über die bestehende Hit-Testing-Logik (`object(atDesignPoint:)`, dieselbe wie bei der Selektion) das Zielobjekt findet — Canvas-Objekte sind alle in einem einzigen `Canvas`-View gezeichnet, nicht als separate SwiftUI-Views, daher kein Drop-Handler pro Objekt möglich. `CanvasStore.assignColor(...)` erzeugt daraus ein neues, unabhängiges `ThreadColor` fürs Zielobjekt und hält `fillColorHex` synchron (Rendering nutzt weiterhin `fillColorHex`, nicht `threadColor`)
    - Palette-Löschen per Swipe (`List.onDelete`) — ohne das wäre eine falsch importierte Datei dauerhaft nicht mehr entfernbar
    - Tests: 2 neue `CanvasStoreTests` für `assignColor` (setzt `threadColor` + passenden `fillColorHex`, unbekannte ID → no-op) — grün (98 Tests gesamt). Kein dedizierter Test für den vollen Drag&Drop-/Datei-Import-UI-Flow (SwiftUI-Fokus-/Drag-Lebenszyklus lässt sich nicht sinnvoll headless testen), aber `GPLPaletteImporterTests` (Phase 7) decken das Parsing bereits gründlich ab
    - Visuell verifiziert (`run-and-screenshot`-Skill): dritter Tab "Garnlisten" erscheint korrekt, leerer Zustand mit "Importieren…"-Button, Klick öffnet den nativen `.fileImporter`-Dialog. Den vollen Import+Drag-Flow mit einer echten `.gpl`-Testdatei liess sich über AppleScript-Koordinatenklicks nicht zuverlässig genug automatisieren (Type-Ahead/Doppelklick im macOS-Spalten-Dateidialog reagierte nicht wie erwartet) — abgebrochen zugunsten der bereits bestehenden Unit-Test-Abdeckung, statt weiter Zeit in UI-Automatisierung zu stecken
  - [x] 8f Einstellungen-Fenster (SwiftUI `Settings`-Scene)
    - `SettingsView.swift` (`SimplStitch/Views/`), gebunden per `@Query` an `AppSettings` (Phase 3-Modell, bisher ohne UI) — `ensureSettingsExist()` legt beim ersten Öffnen die eine erwartete Zeile an (Singleton-in-der-Praxis-Muster, kein eigener Store nötig). Drei Formular-Sektionen: Masseinheit (`preferredMeasurementUnit`, mm/Zoll), Anzahl zuletzt geöffneter Projekte (`maxRecentProjects`, Stepper 1–50), Standard-Garnliste (`defaultThreadPaletteID`, Picker über `@Query(sort: \ThreadPalette.name)` aus dem echten `sharedModelContainer`, "Keine" als expliziter Nil-Fall)
    - `ThreadPalette` bekommt ein neues `id: UUID`-Feld (analog `StitchProject`/`DesignObject`) — ohne das liess sich `defaultThreadPaletteID` (existierte bereits seit Phase 3) nie tatsächlich gegen eine konkrete Palette abgleichen, das Feld war seit drei Phasen unbenutzbar. Additive SwiftData-Schema-Änderung (Default-Wert `UUID()`), kein Migrationscode nötig
    - `SimplStitchApp.swift`: eigene `Settings { SettingsView() }`-Scene neben `DocumentGroup`, gleicher `sharedModelContainer` — SwiftUI ergänzt automatisch den Menüpunkt "Einstellungen…" (⌘,) im App-Menü, keine manuelle Command-Verdrahtung nötig
    - **Scope-Hinweis (aus Issue #2 bewusst nicht übernommen):** Default-Zeichenflächengrösse und Standard-Sticheinstellungen (im Issue nur als Beispiele genannt, nicht als festgelegter Scope) sind nicht Teil von 8f — `AppSettings` hat dafür noch keine Felder, das wäre ein eigener Modell-Erweiterungsschritt; das bestehende `defaultThreadPaletteID`-Feld war dagegen bereits vorhanden und nur unverdrahtet
    - Keine neuen Unit-Tests (reine Formular-Bindings auf ein bereits über `SwiftDataModelsTests` abgedecktes Modell, kein neues Store-Verhalten) — bestehende Suite bleibt grün (98 Tests), `xcodebuild … build` und `xcodebuild … -only-testing:SimplStitchTests test` beide erfolgreich geprüft. Keine visuelle Verifikation (`run-and-screenshot`) durchgeführt, um Token-Verbrauch zu sparen
